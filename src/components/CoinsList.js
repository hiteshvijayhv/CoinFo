            ...
            {coinsList && (
                <div>
                    {coinsList.slice(indexOfFirstPost, indexOfLastPost).map(coin => {
                        ...
                        return (
                            <Link to={`/${coin.id}`} style={{ "textDecoration": "none" }} key={coin.id}>
                            <tr className='table-container' key={coin.id}>
                                <th className='table-item' ><img src={coin.image} alt={coin.name} /></th>
                                <th className='table-item txt' >{coin.name}</th>
                                <th className='table-item txt' ><span>${coin.current_price}</span></th>
                                ...
                                <th className='table-item txt mobile' ><span>{coin.market_cap.toString().slice(0, -6)}</span></th>
                                <th className='table-item txt mobile' ><span>{coin.total_volume.toString().slice(0,-2)}</span></th>
                            </tr></Link>
                        )
                    })}
                </div>
